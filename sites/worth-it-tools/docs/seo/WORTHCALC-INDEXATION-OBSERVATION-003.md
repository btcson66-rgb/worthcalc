...
T0 的 15 個 priority URL URL Inspection 結果為：5 個 `Submitted and indexed`、10 個 `URL is unknown to Google`。這是抽樣觀測，不可外推為全站 1,229 URL 的索引數。
...
目前沒有 fresh Page Indexing URL-level export，因此 CSV 的 `known_google`、`indexed`、`discovered_not_indexed`、`crawled_not_indexed`、`excluded` 全部刻意標為 `UNKNOWN`。不能用 production URL 數、Search Analytics page rows 或 15 URL sample 填補全站 family 數。
...
| T+3 days | 2026-09-16 | pending | PENDING | PENDING | PENDING | PENDING | PENDING | PENDING | PENDING | PENDING | PENDING | PENDING | PENDING | technical stability only |
