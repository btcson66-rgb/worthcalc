...
const slugs = [
  'subscription-price-increase-annual-impact', 'free-trial-auto-renewal-first-charge',
  'pause-vs-cancel-membership-cost', 'family-plan-break-even-members',
  'prepaid-balance-expiration-cost', 'minimum-spend-fee-waiver-break-even',
  'billing-date-cash-flow-overlap',
];
...
for (const [, canonical] of pages) {
  ...
  if (!html.includes('FTC')) failures.push(`${path}: FTC source attribution missing`);
...
if (failures.length) {
  console.error(`SEO program 011 check failed (${failures.length}):`);
  failures.forEach((failure) => console.error(`- ${failure}`));
  process.exit(1);
}
console.log(`SEO program 011 passed: ${pages.length} localized guides have one H1, indexable canonical/robots, Article + BreadcrumbList schema, CFPB/FTC sources, internal links, and sitemap entries.`);
