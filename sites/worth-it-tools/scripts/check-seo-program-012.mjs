...
const slugs = [
  'costco-executive-break-even-eligible-spend',
  'costco-membership-fee-break-even-savings',
  'costco-household-card-break-even-use',
  'costco-gas-detour-break-even',
  'costco-reward-exclusion-effective-rate',
  'costco-reward-cap-usage-break-even',
  'costco-renewal-refund-reward-timing',
];
const locales = ['en', 'es', 'zh', 'hi', 'ar'];
const pages = slugs.flatMap((slug) => locales.map((locale) => [locale, 'https://worthcalc.win/' + locale + '/guides/' + slug + '/']));
const failures = [];
...
for (const [, canonical] of pages) {
  ...
  if (!html.includes('Costco')) failures.push(path + ': Costco attribution missing');
  if (!html.includes('CFPB')) failures.push(path + ': CFPB source attribution missing');
...
if (failures.length) {
  console.error('SEO program 012 check failed (' + failures.length + '):');
  failures.forEach((failure) => console.error('- ' + failure));
  process.exit(1);
}
console.log('SEO program 012 passed: ' + pages.length + ' localized Costco guides have one H1, indexable canonical/robots, Article + BreadcrumbList schema, official-source attribution, internal links, and sitemap entries.');
