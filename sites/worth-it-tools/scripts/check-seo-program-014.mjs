...
const slugs = [
  ...
  'cost-per-use-kitchen-appliance',
  'meal-kit-cost-per-serving-after-waste',
  'coworking-membership-cost-per-day',
  'gym-membership-cost-per-visit',
  'software-seat-cost-per-active-user',
  'hobby-equipment-cost-per-use',
];
...
if (failures.length) {
  console.error('SEO program 014 check failed (' + failures.length + '):');
  failures.forEach((failure) => console.error('- ' + failure));
  process.exit(1);
}
console.log('SEO program 014 passed: ' + pages.length + ' localized cost-per-use guides have one H1, indexable canonical/robots, Article + BreadcrumbList schema, CFPB attribution, internal links, and sitemap entries.');
