...
if (!existsSync(sitemap)) errors.push('dist/sitemap-0.xml missing');
  ...
  for (const locale of locales.filter((candidate) => !isSoftDeindexedUrl(`https://worthcalc.win/${candidate}/guides/${slug}/`))) if (!xml.includes('https://worthcalc.win/' + locale + '/guides/' + slug + '/')) errors.push('/' + locale + '/guides/' + slug + '/: missing sitemap entry');
}
if (errors.length) {
