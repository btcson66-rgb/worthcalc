...
const topics = { '095': 'webcam-electricity-cost-per-meeting-hour-used', '096': 'laptop-docking-station-electricity-cost-per-workday-used', '097': 'external-hard-drive-electricity-cost-per-backup-job-used', '098': 'usb-charging-hub-electricity-cost-per-charging-session-used', '099': 'document-camera-electricity-cost-per-presentation-hour-used', '100': 'digital-photo-frame-electricity-cost-per-display-day-used' };
...
for (const locale of locales) {
  ...
  if (existsSync(source)) { const text = readFileSync(source, 'utf8'); if ((text.match(/^# /gm) || []).length !== 1) errors.push(`${label}: expected one H1`); for (const host of ['eia.gov', 'energystar.gov', 'consumerfinance.gov']) if (!text.includes(host)) errors.push(`${label}: missing ${host}`); if ((text.match(/\]\(\/(?:en|es|zh|hi|ar)\//g) || []).length < 4) errors.push(`${label}: fewer than four internal links`); if (text.length < 1100) errors.push(`${label}: substantive copy is too short`); if (!text.includes('measured') && !text.includes('實測') && !text.includes('medida') && !text.includes('माप') && !text.includes('المقاس')) errors.push(`${label}: missing measurement boundary`); }
