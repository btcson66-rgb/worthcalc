...
const topics = {
  113: 'aquarium-heater-electricity-cost-per-tank-day-used',
  114: 'terrarium-heat-lamp-electricity-cost-per-heating-hour-used',
  115: 'electric-insect-zapper-electricity-cost-per-overnight-used',
  116: 'outdoor-string-lights-electricity-cost-per-evening-used',
  117: 'garage-door-opener-electricity-cost-per-cycle-used',
  118: 'electric-fireplace-electricity-cost-per-heating-hour-used',
};
...
for (const locale of locales) {
  ...
  if (existsSync(source)) {
    ...
    if (!['measured', '實測', 'medida', 'mide', 'medidos', 'माप', 'المقاس'].some((term) => lower.includes(term.toLowerCase()))) errors.push(`${label}: missing measurement boundary`);
  }
