---
contentType: article
articleSlug: "desk-fan-electricity-cost-per-cooling-hour-used"
locale: "es"
title: "Coste eléctrico de un ventilador de mesa por hora de uso: incluye velocidad y oscilación"
description: "Estima el coste eléctrico de un ventilador de mesa por hora con energía medida según velocidad, oscilación, temporizador y espera, separando el aire acondicionado."
relatedTool: "/es/tools/budget-builder/"
canonical: "https://worthcalc.win/es/guides/desk-fan-electricity-cost-per-cooling-hour-used/"
lastReviewed: "2026-09-03"
draft: false
---

# Coste eléctrico de un ventilador de mesa por hora de uso: incluye velocidad y oscilación

> **Quick answer:** Estima el coste eléctrico de un ventilador de mesa por hora con energía medida según velocidad, oscilación, temporizador y espera, separando el aire acondicionado. This is a cost estimate, not a savings, performance, ranking, or outcome guarantee.

## Qué cambia el resultado

El ventilador mueve aire, pero no equivale a una reducción medida de temperatura. Define la hora como hora de funcionamiento y no prometas un resultado térmico. Usa un medidor de pared y declara el inicio y el final exactos. Registra modo, carga, accesorios, unidad terminada, tarifa y fecha. La potencia nominal sirve para planear, pero no sustituye una lectura repetible.

## Fórmula y ejemplo

Coste eléctrico = kWh medidos en la pared × tarifa eléctrica entregada. Coste unitario = coste eléctrico ÷ unidades terminadas. Mantén cuotas fijas, impuestos, consumibles, reparaciones, suscripciones y otros equipos fuera del numerador salvo que el límite los incluya. Con 0,028 kWh por hora y 0,18 por kWh, el coste es 0,00504 por hora. No es el coste de enfriar toda una habitación.

Do not multiply rated wattage by the full elapsed time unless the meter confirms that load. Measure computers, displays, networks, chargers, and other accessories separately before combining them.

## Compara usos reales

¿Cambia la oscilación el resultado? Mídela como condición aparte y no la presentes como garantía térmica. Related guides: [Coste eléctrico de un masajeador de pies por sesión: separa espera y calor](/es/guides/electric-foot-massager-electricity-cost-per-massage-session-used/), [Coste eléctrico de un proyector por hora de presentación: incluye calentamiento y enfriamiento](/es/guides/projector-electricity-cost-per-presentation-hour-used/), [Coste eléctrico de un lector electrónico por hora de lectura: cuenta la carga](/es/guides/e-reader-electricity-cost-per-reading-hour-used/), and [Coste eléctrico de un sacapuntas eléctrico por sesión: cuenta los ciclos](/es/guides/electric-pencil-sharpener-electricity-cost-per-sharpening-session-used/). Each answers a different unit-cost question, so select the denominator and boundary before comparing.

## Límites y errores comunes

El voltaje, la tarifa, el firmware, las condiciones, el modo, la carga, la resolución del medidor, la espera y los fallos cambian el resultado. Haz tres pruebas repetibles como mínimo y cambia una condición por vez. For comparison, keep the tariff date and measurement boundary fixed.

## Preguntas frecuentes

### ¿Cambia la oscilación el resultado? Mídela como condición aparte y no la presentes como garantía térmica.

¿Cambia la oscilación el resultado? Mídela como condición aparte y no la presentes como garantía térmica.

## Fuentes

Para el contexto de tarifas, consulta la [FAQ de precios eléctricos entregados de EIA](https://www.eia.gov/tools/faqs/faq.php?id=507) y el [resumen de electricidad doméstica de EIA](https://www.eia.gov/energyexplained/use-of-energy/electricity-use-in-homes.php). [ENERGY STAR computadores](https://www.energystar.gov/products/computers) y [equipos de imagen](https://www.energystar.gov/products/imaging_equipment) aportan contexto de categoría y gestión de energía, no una promesa para este equipo. La [guía de gastos de CFPB](https://www.consumerfinance.gov/owning-a-home/prepare/figure-out-how-much-you-want-to-spend/) ayuda a usar una medición como entrada de decisión.
